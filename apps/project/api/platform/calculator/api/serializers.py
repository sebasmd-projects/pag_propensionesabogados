...
class ClientDataSerializer(serializers.ModelSerializer):
    ...
    documentNumber = serializers.CharField(source='user.document_number', read_only=True)
    firstName = serializers.CharField(source='debtor_first_name')
    lastName = serializers.CharField(source='debtor_last_name')
    email = serializers.EmailField(source='debtor_email', required=False, allow_blank=True)
    phone = serializers.CharField(source='debtor_cell_phone', required=False, allow_blank=True)
    address = serializers.CharField(source='debtor_address', required=False, allow_blank=True)
    birthDate = serializers.DateField(source='debtor_birth_date')

    class Meta:
        model = AttlasInsolvencyFormModel
        fields = ['id', 'documentNumber', 'firstName', 'lastName', 'email', 'phone', 'address', 'birthDate']
...
class ClientCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AttlasInsolvencyFormModel
        fields = ['documentNumber', 'birthDate', 'firstName', 'lastName', 'email', 'phone', 'address']
...
class ClientUpdateSerializer(serializers.ModelSerializer):
    firstName = serializers.CharField(source='debtor_first_name', required=False)
